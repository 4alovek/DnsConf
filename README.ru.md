...
### Исключение целых секций hosts-файла

Чтобы не загружать ненужные сервисы из большого hosts-файла, добавьте названия их секций в файл `hosts_exclude` в корне репозитория — по одному названию на строку. Можно указывать название как с `#`, так и без него; регистр и пробелы по краям не учитываются.
...
При обработке секции `# Naukri` или `# Bike24` все строки до следующего заголовка-комментария будут пропущены. Домены из исключённых секций, ранее добавленные в NextDNS, удалятся при следующем запуске.
...
+ Домены, которых больше нет в источниках `REDIRECT` (включая исключённые через `hosts_exclude` секции), будут удалены
